...
(function () {
  ...
  /* olapp.source.Gist */
  olapp.source.Gist = new olapp.Source('Gist');
...
'</li>';
      ...
      obj.children('button').click(function () {
        if ($(this).hasClass('btn-primary')) {    // Add layer
          var layerOptions = {
            title: gist.filename,
            olapp: {
              source: sourceName
        ...
        else {    // Remove/Restore URL button
    ...
    function appendNewItem(url, filename, description) {
      parent.append(createListItem(storage.append(url, filename, description)));
    }
...
'  </div>' +
'</li>';
    var listItem = $(html).appendTo(parent);
    listItem.find('button').click(function () {
      var url = listItem.find('input').val();
      if (!url) return;
    ...
    storage.gists().forEach(function (gist) {
      parent.append(createListItem(gist));
    });
  };
